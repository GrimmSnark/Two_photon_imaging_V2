...
for frameNum=1:nFrames
    ...
    for i=1:length(dots)  %Loop through the fields
        ...
        pixpos.x(id) = degreeVisualAngle2Pixels(display.displaySetup,dots(i).x) + display.resolution(1)/2;
        pixpos.y(id) = degreeVisualAngle2Pixels(display.displaySetup,dots(i).y)+ display.resolution(2)/2;
