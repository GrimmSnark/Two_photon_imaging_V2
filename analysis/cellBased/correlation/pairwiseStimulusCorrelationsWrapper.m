function  pairwiseStimulusCorrelationsWrapper(filepaths)
    ...
    for i = 1:length(filepathList)
        ...
        try
            if length(experimentStructure.cndTotal) > 8
                pairwiseStimulusCorrelations(experimentStructure, 33:40);
            else
                pairwiseStimulusCorrelations(experimentStructure, 1:8);
            end
        catch
        end
    end
