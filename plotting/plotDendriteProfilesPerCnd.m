...
if ~exist([experimentStructure.savePath 'RawLinePic\dendrites\dendriteStructure.mat'], 'file')
    ...
    load([experimentStructure.savePath 'RawLinePic\dendrites\dendriteStructure.mat']);
end
...
ij.IJ.runMacro('setTool("Oval");');

%% Deal with ROI selection

% Check if there are already ROIs selected for this recording

if exist([experimentStructure.savePath 'RawLinePic\dendrites\dendriteROIs.zip'], 'file')
    disp([experimentStructure.savePath  ' contains a valid ROI file!']);
    RC.runCommand('Open', [experimentStructure.savePath 'RawLinePic\dendrites\dendriteROIs.zip']); % opens ROI file
    
    % Query user if you want to use previously chosen ROIs
    answer = MFquestdlg([0.5,0.5], 'Would you like to load previously chosen dendrite ROIs?', ...
        'Choose your ROIs', ...
        'Yes','No', 'Yes');
    % Handle response
    switch answer
...
questText = [{'Choose new ROIs to analyse'} ...
    {'Use the oval tool to select lines and press "t" to add to ROI manager'} {''} ...
...
ROInumber = length(RC.getRoisAsArray()); % get pointers to all line ROIs
dendriteStructure.cellCount = ROInumber;
...
for x = 1:ROInumber
    ...
    dendriteStructure.yPos(x) =  currentROI.getYBase;
    dendriteStructure.xPos(x) = currentROI.getXBase;
    
    ij.IJ.getInstance().toFront();
        
    % Get the fluorescence timecourse for the ROI by
    % using ImageJ's "z-axis profile" function.
        ...
        dendriteStructure.rawF(x,:) = RT(:,2);
end

%% Split data into conditions

analysisFrameLength = experimentStructure.meanFrameLength ;
dendriteStructure.meanFrameLength = analysisFrameLength;
    ...
    for  x =1:length(experimentStructure.cndTotal) % for each condition
        if any(experimentStructure.cndTotal(x)) % checks if there are any trials of that type
            for y =1:length(experimentStructure.cndTrials{x}) % for each trial of that type
                ...
                dendriteStructure.rawFperCnd{p}{x}(:,y) = dendriteStructure.rawF(p,currentTrialFrameStart:currentTrialFrameStart+ (analysisFrameLength-1)); %chunks data and sorts into structure
                
                % calulates per trial DF/F FBS
                rawFCurrentTrial = dendriteStructure.rawF(p,currentTrialFrameStart:currentTrialFrameStart+ (analysisFrameLength-1)); % gets the raw trial
                rawFCurrentFrameBeforeStim = dendriteStructure.rawF(p,currentTrialFrameStart+experimentStructure.stimOnFrames(1)-2); % get the FBS values
                dendriteStructure.dFperCndFBS{p}{x}(:,y) = (rawFCurrentTrial - rawFCurrentFrameBeforeStim)/rawFCurrentFrameBeforeStim; %creates the trial dF/F for FBS
                
                % makes average of prestim window
                dendriteStructure.dFpreStimWindowFBS{p}{y,x} =  dendriteStructure.dFperCndFBS{p}{x}(1:experimentStructure.stimOnFrames(1)-1,y);
                dendriteStructure.dFpreStimWindowAverageFBS{p}{y,x} = mean(dendriteStructure.dFpreStimWindowFBS{p}{y,x});
                
                % stim response and average response per cell x cnd x trial
                dendriteStructure.dFstimWindowFBS{p}{y,x} =  dendriteStructure.dFperCndFBS{p}{x}(experimentStructure.stimOnFrames(1):experimentStructure.stimOnFrames(2),y);
                dendriteStructure.dFstimWindowAverageFBS{p}{y,x} = mean(dendriteStructure.dFstimWindowFBS{p}{y,x});
            end
        end
    end
end
...
for i = 1:length(dendriteStructure.rawFperCnd) % for each cell
    for x = 1:length(experimentStructure.dFperCnd{i}) % for each condition
          %% First before Stimulus
        dendriteStructure.dFperCndMeanFBS{i}(:,x) = mean(dendriteStructure.dFperCndFBS{i}{x}, 2); % means for each cell frame value x cnd
        dendriteStructure.dFperCndSTDFBS{i}(:,x) = std(dendriteStructure.dFperCndFBS{i}{x}, [], 2); % std for each cell frame value x cnd
    end
end
