...
% MODIFED TO ALLOW HARD SET OF xlims
...
if nin > 4 % nin == 4, all params case
    [~,nbins,dist,plotHist, normNo] = varargin{:};
    dist = convertStringsToChars(dist);
elseif nin == 4 % nin == 4, all params case
    [~,nbins,dist,plotHist] = varargin{:};
    dist = convertStringsToChars(dist);
    normNo = [];
elseif nin == 3 % nin == 3, all params case
    [~,nbins,dist] = varargin{:};
    dist = convertStringsToChars(dist);
    plotHist = [];
    normNo = [];
elseif nin == 2 
    dist=[];nbins = varargin{2}; plotHist = []; normNo = [];
else
    nbins=[];dist=[]; plotHist = []; normNo = [];
end
...
if plotHist ==1
    if isempty(normNo)
        ...
        if binwidth < 1
            hh = bar(ax, bincenters, bincounts/normNo/ binwidth,1);
        else
            hh = bar(ax, bincenters, bincounts/normNo*binwidth,1);
        end
    end
