...
% Modified M Savage Oct 2019 to allow specification of axis to plot in
%
...
% 'Axis'- Axis handle to plot within
%
%
%
% Outputs
...
if isempty(axis2Use)
    axis2Use = gca;
    initialHoldStatus=ishold(axis2Use);
else
    initialHoldStatus=ishold(axis2Use);
end
...
function H = makePlot(x,y,errBar,lineProps,transparent,patchSaturation, axis2Use)
    ...
    if(isdatetime(x))
        H.patch=patch(axis2Use,datenum(xP),yP,1);
    else
        H.patch=patch(axis2Use, xP,yP,1);
    end
